...
hlint-refactor:
  ...
  exec nix develop --quiet --command bash -c \
    'set -euo pipefail
     ...
     done < <(find components tooling bin core-libs scripts/nix/ucd2haskell-aihc test/support -name "*.hs" -not -path "*/dist-newstyle/*" -not -path "*/test/Test/Fixtures/*" -print0)'
...
check:
  ...
  nix develop --quiet --command bash -c 'set -euo pipefail; bindings_directory=$(mktemp -d); trap '\''rm -rf "$bindings_directory"'\'' EXIT; wit-bindgen c --world command --out-dir "$bindings_directory" components/aihc-wasm/runtime/wit; while IFS= read -r -d "" file; do if [[ "$file" == *components/aihc-wasm/runtime/aihc_wasip3.c ]]; then clang-tidy-unwrapped --quiet "$file" -- --target=wasm32-unknown-unknown -std=c11 -ffreestanding -Wall -Wextra -Wpedantic -DAIHC_WASIP3 -Icomponents/aihc-wasm/runtime/include -Icomponents/aihc-native/runtime -isystem "$bindings_directory"; else clang-tidy --quiet "$file" -- -std=c11 -Wall -Wextra -Wpedantic; fi; done < <(find components tooling bin core-libs scripts test -type f -name "*.c" -not -path "*/dist-newstyle/*" -print0)'
