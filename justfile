...
check:
  ...
  nix develop --quiet --command bash -c 'find components tooling bin core-libs scripts test -type f \( -name "*.c" -o -name "*.h" \) -not -path "*/dist-newstyle/*" -not -path "*/test/Test/Fixtures/*" -not -path "*/generated/*" -print0 | xargs -0 -r clang-format --dry-run --Werror'
  nix develop --quiet --command bash -c 'set -euo pipefail; while IFS= read -r -d "" file; do if [[ "$file" == *core-libs/aihc-rts/wasm/*.c || "$file" == *aihc_host_wasip3.c ]]; then clang-tidy-unwrapped --quiet "$file" -- --target=wasm32-wasip1 --sysroot="$AIHC_WASM_SYSROOT" -std=c11 -Wall -Wextra -Wpedantic -Icore-libs/aihc-rts/wasm -Icore-libs/aihc-rts/native -isystem core-libs/aihc-rts/wasm/generated; else clang-tidy --quiet "$file" -- -std=c11 -Wall -Wextra -Wpedantic -Icore-libs/aihc-rts/native; fi; done < <(find components tooling bin core-libs scripts test -type f -name "*.c" -not -path "*/dist-newstyle/*" -not -path "*/test/Test/Fixtures/*" -not -path "*/generated/*" -print0)'
  nix develop --quiet --command scripts/update-wit-bindings.sh --check
