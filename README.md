...
| Self-compile       | <!-- AUTO-GENERATED: START self-hosting-progress --> `38/130` (`29.23%`) ●○○○○ <!-- AUTO-GENERATED: END self-hosting-progress --> |
| &nbsp; | &nbsp; |
...
<details>
<summary>Self-compile packages: 38 install, 13 fail, 79 wait for a dependency</summary>
...
| Cabal-syntax | 3.14.2.0 | ⏸️ needs `binary`, `containers`, `directory`, `parsec`, `text` |
...
| process | 1.6.26.1 | ⏸️ needs `directory`, `unix` |
| Cabal | 3.14.2.0 | ⏸️ needs `Cabal-syntax`, `containers`, `directory`, `parsec`, `process`, `unix` |
| crypton-x509-system | 1.8.0 | ⏸️ needs `containers`, `crypton-pem`, `crypton-x509`, `crypton-x509-store`, `directory`, `process` |
| haskeline | 0.8.4.1 | ⏸️ needs `containers`, `directory`, `process`, `unix` |
...
The "Self-compile" row counts the packages of the `aihc` executable that `aihc install` can compile, with `aihc` itself as the last package. [docs/self-hosting-packages.md](docs/self-hosting-packages.md) lists the packages. A package waits when one of its dependencies does not install.
