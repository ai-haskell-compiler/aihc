  ...
  defaultMain . testGroup "aihc" $
    [ testGroup
        ...
        [ testCase "writes no legacy Core files and reuses equal SCC inputs" test_installV2ResolveArtifacts,
          ...
          testCase "keeps GRIN for a source fixture" test_installV2KeepGrin,
...
test_installV2KeepGrin = do
  sourceRoot <- findFixtureRoot "bin/aihc/test/Test/Fixtures/install-v2/keep-grin"
  withTempDir "aihc-install-v2-keep-grin" $ \root -> do
    let options = InstallV2Options sourceRoot (Just (root </> "store")) False True
    result <- installV2 options
    let path = installV2StorePath result </> "Demo" </> "grin"
    assertFileExists path
    source <- TIO.readFile path
    case Grin.parseProgram source of
      Left parseError -> assertFailure ("invalid GRIN file " <> path <> ": " <> Grin.renderParseError parseError)
      Right program -> assertEqual "GRIN lint errors" [] (Grin.lintProgram program)
...
test_installV2AihcPrim = do
  ...
  withTempDir "aihc-install-v2-aihc-prim" $ \root -> do
    let storeRoot = root </> "store"
        options = InstallV2Options aihcPrimRoot (Just storeRoot) False True
    ...
    assertEqual "GRIN file count" (length aihcPrimLibraryModules) (length grinFiles)
    mapM_ assertGrinFile grinFiles
...
assertGrinFile :: FilePath -> Assertion
assertGrinFile path = do
  source <- TIO.readFile path
  case Grin.parseProgram source of
    Left parseError -> assertFailure ("invalid GRIN file " <> path <> ": " <> Grin.renderParseError parseError)
    Right program -> assertEqual "GRIN lint errors" [] (Grin.lintProgram program)
