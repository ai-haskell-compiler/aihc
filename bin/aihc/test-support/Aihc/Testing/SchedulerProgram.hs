...
stdioSchedulerProgram =
    ...
    { grinConstructors = [pubConstructor "()" []],
      grinPrimitives =
        [ (GrinVar "awaitIO#" 30 lifted, 2),
          ...
          (GrinVar "submitIORead#" 43 (BoxedRep Unlifted), 4),
          (GrinVar "submitIOWrite#" 44 (BoxedRep Unlifted), 4),
          (GrinVar "touch#" 45 lifted, 2),
          (GrinVar "stdinIOHandle#" 46 (BoxedRep Unlifted), 0),
          (GrinVar "stdoutIOHandle#" 47 (BoxedRep Unlifted), 0),
          (GrinVar "takeIOResult#" 48 IntRep, 1)
        ],
      grinForeignCalls = [],
              ...
              grinFunctionBody =
                GrinBind [byteArray] (GrinPrimitiveCall (BoxedRep Unlifted) "newPinnedByteArray#" [intValue 64]) $
                  GrinBind [bufferContents] (GrinPrimitiveCall AddrRep "mutableByteArrayContents#" [GrinVarValue byteArray]) $
                    GrinBind [stdinIOHandle] (GrinPrimitiveCall (BoxedRep Unlifted) "stdinIOHandle#" []) $
                      GrinBind [readRequest] (GrinPrimitiveCall (BoxedRep Unlifted) "submitIORead#" [GrinVarValue stdinIOHandle, GrinVarValue bufferContents, intValue 0, intValue 64]) $
                        GrinBind [] (GrinPrimitiveCall (TupleRep []) "awaitIO#" [GrinVarValue readRequest]) $
                          GrinBind [readCount] (GrinPrimitiveCall IntRep "takeIOResult#" [GrinVarValue readRequest]) $
                            GrinBind [stdoutIOHandle] (GrinPrimitiveCall (BoxedRep Unlifted) "stdoutIOHandle#" []) $
                              GrinBind [writeRequest] (GrinPrimitiveCall (BoxedRep Unlifted) "submitIOWrite#" [GrinVarValue stdoutIOHandle, GrinVarValue bufferContents, intValue 0, GrinVarValue readCount]) $
                                GrinBind [] (GrinPrimitiveCall (TupleRep []) "awaitIO#" [GrinVarValue writeRequest]) $
                                  GrinBind [writeResult] (GrinPrimitiveCall IntRep "takeIOResult#" [GrinVarValue writeRequest]) $
  ...
  where
    ...
    stdinIOHandle = GrinVar "stdin_handle" 36 (BoxedRep Unlifted)
    readRequest = GrinVar "read_request" 37 (BoxedRep Unlifted)
    readCount = GrinVar "read_count" 38 IntRep
    stdoutIOHandle = GrinVar "stdout_handle" 39 (BoxedRep Unlifted)
    writeRequest = GrinVar "write_request" 40 (BoxedRep Unlifted)
