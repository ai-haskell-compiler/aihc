...
prop_symbolRoundTrip = property $ do
  ...
  annotate (BS8.unpack (SBS.fromShort rendered))
...
decodeLinkedSymbol :: SBS.ShortByteString -> Either String BS.ByteString
decodeLinkedSymbol rendered = BS.pack <$> go (BS8.unpack (SBS.fromShort rendered))
  where
    go [] = Right []
