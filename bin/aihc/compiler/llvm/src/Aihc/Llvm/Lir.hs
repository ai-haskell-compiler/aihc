...
quoteBytes :: BS.ByteString -> Text
quoteBytes name = "\"" <> T.concat (map escape (BS.unpack name)) <> "\""
  where
    escape byte
      | byte == 0x22 = "\\22"
      | byte == 0x5c = "\\5C"
      | byte < 0x20 || byte > 0x7e = "\\" <> hexByte byte
      | otherwise = T.singleton (toEnum (fromIntegral byte))
...
renderSymbol symbol = "@" <> quoteBytes (SBS.fromShort (unSymbol symbol))
