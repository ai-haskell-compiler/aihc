...
tests = do
  ...
  failures <- failureFixtures
  pure (testGroup "arm64" [suite, assembly, slotReloadTests, failures])
...
failureFixtures :: IO TestTree
failureFixtures = do
  root <- fromMaybe "." <$> lookupEnv "AIHC_TEST_ROOT"
  let directory = root </> "bin/aihc/compiler/arm64/test/Test/Fixtures/object-fail"
  ...
  where
    ...
    checkFailure (FailureFixture language expected source) = do
      compile <- case language of
        "lir" -> Arm64.writeLirObjectWith False <$> checked (Lir.parseModule source)
        "grin" -> do
          program <- checked (Grin.parseProgram source)
          gc <- Grin.lowerGc <$> checked (Grin.toCpsGrin program)
          pure (Arm64.writeGrinObjectWith False False Nothing gc)
      ...
      forM_ [False, True] $ \existing ->
        withSystemTempDirectory "aihc-object-failure" $ \directory -> do
          ...
          result <- try (compile path) :: IO (Either SomeException ())
          case result of
