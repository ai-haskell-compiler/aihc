...
`aihc build --no-link` and `aihc build-module --no-link` stop before the link
and write a bundle directory instead of an executable. The bundle holds a copy of every link
...
`aihc build`, `aihc build-module`, and `aihc install` take `-O LEVEL`.
...
`aihc build-module -O0` builds its modules and its packages at level 0, so the first unoptimized build of a store also builds `aihc-base` at level 0.
...
`aihc build-module` resolves its `--package` constraints through the same plan.
...
`aihc build` takes a Cabal package instead of a main module: a local directory, or a Hackage name with an optional version, as `aihc install` takes it.
It builds every executable of the package whose `buildable` flag is set, each from the `main-is`, `other-modules`, `default-extensions`, and `build-depends` of its own stanza.
The `build-depends` resolve through the same plan as the `--package` constraints of `aihc build-module`, with the package itself and its siblings found before Hackage, so an executable that depends on the library of its package builds that library first.
