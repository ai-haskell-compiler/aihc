              ...
              source =
                unlines
                  [ "#include \"aihc_runtime.h\"",
                    ...
                    "  if (aihc_mutvar_compare_and_swap(first, (AihcSlot)leaf, (AihcSlot)second) != 1) return 0;",
                    "  if (aihc_mutvar_compare_and_swap(first, (AihcSlot)leaf, (AihcSlot)first) != 0) return 0;",
...
testNativeScheduler = do
  ...
  assertBool "captures argc and argv before machine startup" ("bl _aihc_program_arguments_initialize" `T.isInfixOf` assembly)
...
testNativeSynchronousException = do
  ...
  assembly <-
    case compileProgram "main" gc of
      Right value -> pure value
      Left err -> assertFailure ("ARM64 exception lowering failed: " <> show err)
  assertBool "emits the shared raise transfer" ("bl _aihc_raise" `T.isInfixOf` assembly)
  when (arch == "aarch64" && os == "darwin") $
