...
static int32_t aihc_wasip3_start_output(const unsigned char *bytes,
                                        size_t length, int complete_io) {
  ...
  aihc_output.complete_io = complete_io;
...
int32_t aihc_wasip3_start_write(const unsigned char *bytes, size_t length) {
  return aihc_wasip3_start_output(bytes, length, 1);
}
...
int puts(const char *string) {
  ...
  unsigned char *line = malloc(length + 1);
  if (line == NULL) {
    return -1;
  }
  memcpy(line, string, length);
  line[length] = '\n';
  int32_t result = aihc_wasip3_start_output(line, length + 1, 0);
  return result == -1 ? -1 : 0;
}
...
static command_callback_code_t aihc_pump(void) {
  ...
  if (aihc_wasm_finished && !aihc_output.active) {
...
exports_wasi_cli_run_run_callback(command_event_t *event) {
  (void)event;
  int complete_io = aihc_output.complete_io;
