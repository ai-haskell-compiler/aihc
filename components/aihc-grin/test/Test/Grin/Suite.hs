      ...
      if runtimeRep == BoxedRep Lifted
        then Right (program, bindingEntry (varType var), id)
          ...
          let componentCount = length (runtimeRepComponents runtimeRep)
              ...
              declaration = FcData "__AihcEvalResultType" [] [fcDataCon "__AihcEvalResultType" constructorName [varType var]]
...
applicationProgram =
  FcProgram
    [ FcData "BoxedInt" [] [fcDataCon "BoxedInt" "BoxedInt" [intTy]],
      FcTopBind
        ( FcNonRec
            answerVar
            ( FcApp
...
hiddenLambdaProgram aliasRhs =
  FcProgram
    [ FcData "Holder" [] [fcDataCon "Holder" "Holder" [binaryFunctionTy]],
      FcTopBind
        ( FcNonRec
            dictionaryVar
...
sharedForcedLetProgram =
  FcProgram
    [ FcData "FunctionPair" [] [fcDataCon "FunctionPair" "FunctionPair" [unaryFunctionTy, unaryFunctionTy]],
      FcTopBind
        ( FcNonRec
            ownerVar
            ( FcLam inputVar $
...
zeroWidthConstructorProgram =
  FcProgram
    [ FcData "StateBox" [] [fcDataCon "StateBox" "StateBox" [stateTy, boxedIntTy]],
      FcTopBind
        ( FcNonRec
            partialVar
...
knownFunctionWrapperProgram isEtaExpansion =
  FcProgram
    [ FcData "Applicative" [] [fcDataCon "Applicative" "$Dict$Applicative" [unaryFunctionTy]],
      FcTopBind
        ( FcNonRec
            pureIoVar
...
dictionaryProgram =
  FcProgram
    [ FcData "Box" [] [fcDataCon "Box" "Box" [intTy]],
      FcData "Test" [] [fcDataCon "Test" "$Dict$Test" [boxedIntTy, boxedIntTy]],
      FcTopBind
        ( FcNonRec
            answerVar
            ( FcCase
...
foreignProgram =
  FcProgram
    [ FcData "BoxedInt32" [] [fcDataCon "BoxedInt32" "BoxedInt32" [int32Ty]],
...
saturatedConstructorProgram =
  FcProgram
    [ FcData "Int32" [] [fcDataCon "Int32" "I32#" [int32Ty]],
      FcTopBind
        ( FcNonRec
            int32WrapperVar
...
separateConstructorPrograms =
  [ FcProgram [FcData "Int32" [] [fcDataCon "Int32" "I32#" [int32Ty]]],
    FcProgram
      [ FcTopBind
          ( FcNonRec
              wrapperVar
...
fcDataCon :: Text -> Text -> [TcType] -> FcDataConstructor
fcDataCon typeName name fields = FcDataConstructor name (foldr TcFunTy (TcTyCon (TyCon typeName 0) []) fields)
