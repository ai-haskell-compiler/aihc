...
-- | Renderable annotations for type-checker golden overlays.
module Aihc.Tc.RenderedAnnotations
  ( annotateRenderedTcLabels,
    tcModuleDiagnostics,
  )
where
...
import Aihc.Tc.Types (Kind (..), Pred (..), TcType (..), TyCon (..), Unique (..))
...
annotateRenderedTcLabels :: [TcDiagnostic] -> Module -> Module
annotateRenderedTcLabels diagnostics modu =
  attachRenderedLabels (collectTcLabels modu diagnostics) (attachDiagnostics diagnostics modu)
...
attachDiagnostics :: [TcDiagnostic] -> Module -> Module
attachDiagnostics diagnostics modu =
  case moduleDecls modu of
    [] -> modu {moduleAnns = moduleAnns modu <> map mkAnnotation diagnostics}
    decl : decls ->
      modu {moduleDecls = foldr attachDiagnostic decl diagnostics : decls}
  where
    attachDiagnostic diagnostic decl =
      DeclAnn
        (mkAnnotation diagnostic)
        (DeclAnn (mkAnnotation (diagnosticSpan diagnostic)) decl)
...
attachRenderedLabels labels modu =
  case moduleDecls modu of
    [] -> modu
    decl : decls ->
      modu {moduleDecls = foldr attachLabel decl concreteLabels : decls}
  where
    concreteLabels = filter ((/= NoSourceSpan) . labelSpan) labels

    attachLabel label decl =
      DeclAnn
        (mkAnnotation (TcRenderedAnnotation (labelText label)))
        (DeclAnn (mkAnnotation (labelSpan label)) decl)

collectTcLabels :: Module -> [TcDiagnostic] -> [TcLabel]
collectTcLabels modu diagnostics =
  sortOn labelKey $
    ...
    concatMap (declLabels Nothing) (moduleDecls modu)
      <> diagnosticLabels diagnostics
