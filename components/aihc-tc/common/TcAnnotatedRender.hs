  ...
  case compare (length sources) (length results) of
    ...
    EQ ->
      let moduleSources = sortOn (moduleDisplayName . snd) (zip sources results)
       in renderAnnotatedModuleSources renderTcLabel (map fst moduleSources) (map (materializeTcLabels . snd) moduleSources)

moduleDisplayName :: Module -> Text
moduleDisplayName modu = fromMaybe "<unnamed>" (moduleName modu)

newtype TcRenderedLabel = TcRenderedLabel String
  deriving (Eq, Show)

renderTcLabel :: Annotation -> Maybe (Doc ann)
renderTcLabel annotation = do
  TcRenderedLabel label <- fromAnnotation annotation
  pure (pretty label)
...
materializeTcLabels :: Module -> Module
materializeTcLabels modu =
  evalState (placeTcLabels modu) (collectTcLabels modu)

placeTcLabels :: forall a. (Data a) => a -> State [TcLabel] a
placeTcLabels value = do
  value' <- gmapM placeTcLabels value
  placeTcLabelsAtNode value'

placeTcLabelsAtNode :: forall a. (Data a) => a -> State [TcLabel] a
placeTcLabelsAtNode value =
  fromMaybe (pure value) $
    placeAs @Name placeNameLabels value
      <|> placeAs @UnqualifiedName placeUnqualifiedNameLabels value
      <|> placeWrapperAs @Decl DeclAnn value
      <|> placeWrapperAs @Expr EAnn value
      <|> placeWrapperAs @Pattern PAnn value
      <|> placeWrapperAs @Type TAnn value
      <|> placeWrapperAs @Literal LitAnn value
      <|> placeWrapperAs @DataConDecl DataConAnn value
      <|> placeWrapperAs @GuardQualifier GuardAnn value
      <|> placeWrapperAs @(DoStmt Expr) DoAnn value
      <|> placeWrapperAs @(DoStmt Cmd) DoAnn value
      <|> placeWrapperAs @CompStmt CompAnn value
      <|> placeWrapperAs @ArithSeq ArithSeqAnn value
      <|> placeWrapperAs @ClassDeclItem ClassItemAnn value
      <|> placeWrapperAs @InstanceDeclItem InstanceItemAnn value
      <|> placeWrapperAs @Cmd CmdAnn value
      <|> placeWrapperAs @ExportSpec ExportAnn value
      <|> placeWrapperAs @ImportItem ImportAnn value

placeAs :: forall node a. (Typeable node, Typeable a) => (node -> State [TcLabel] node) -> a -> Maybe (State [TcLabel] a)
placeAs place value = do
  node <- cast value
  pure (replaceNode <$> place node)

placeWrapperAs ::
  forall syntax a.
  (Data syntax, Typeable a) =>
  (Annotation -> syntax -> syntax) ->
  a ->
  Maybe (State [TcLabel] a)
placeWrapperAs wrap =
  placeAs @syntax (placeWrapperLabels wrap)

replaceNode :: forall b a. (Typeable b, Typeable a) => b -> a
replaceNode new =
  case cast new of
    Just value -> value
    Nothing -> error "materializeTcLabels: internal annotation cast failed"

placeNameLabels :: Name -> State [TcLabel] Name
placeNameLabels name = do
  labels <- takeLabelsAt (spanFromAnnotations (nameAnns name))
  pure (name {nameAnns = nameAnns name <> map labelAnnotation labels})

placeUnqualifiedNameLabels :: UnqualifiedName -> State [TcLabel] UnqualifiedName
placeUnqualifiedNameLabels name = do
  labels <- takeLabelsAt (spanFromUnqualifiedName name)
  pure (name {unqualifiedNameAnns = unqualifiedNameAnns name <> map labelAnnotation labels})

placeWrapperLabels :: (Data syntax) => (Annotation -> syntax -> syntax) -> syntax -> State [TcLabel] syntax
placeWrapperLabels wrap syntax = do
  labels <- takeLabelsAt (concreteSpan (firstSourceSpan syntax))
  pure (foldr (wrap . labelAnnotation) syntax labels)

labelAnnotation :: TcLabel -> Annotation
labelAnnotation =
  mkAnnotation . TcRenderedLabel . labelText

takeLabelsAt :: Maybe SourceSpan -> State [TcLabel] [TcLabel]
takeLabelsAt Nothing = pure []
takeLabelsAt (Just sp) =
  state $ \labels ->
    let (matching, remaining) = partition (sameStart sp . labelSpan) labels
     in (matching, remaining)

sameStart :: SourceSpan -> SourceSpan -> Bool
sameStart (SourceSpan _ line col _ _ _ _) (SourceSpan _ line' col' _ _ _ _) =
  line == line' && col == col'
sameStart _ _ = False
    ...
    ownSpan =
      fromMaybe NoSourceSpan $
        (spanFromAnnotation <$> cast node)
          <|> (fromMaybe NoSourceSpan . spanFromAnnotations <$> cast node)
