...
module Aihc.Fc2.Desugar.Core.Expr
  ( dsMatches,
    dsMatchesWithDicts,
...
import Aihc.Fc2.Desugar.Core.Lower (seqPseudoOpName)
import Aihc.Fc2.Desugar.Core.Match (dsPatternPure, numericRuntimeRep)
import Aihc.Fc2.Desugar.Core.Subst (OccurrenceCount (..), countExprVar, substExpr, substExprVar, substType)
...
data DsState = DsState
  { dsNextUnique :: !Int,
    ...
    dsGlobalVars :: !(Map CoreSymbolOrigin Var),
    ...
    dsTupleConstructorOrigin :: !(Maybe CoreSymbolOrigin)
  }
...
primitiveDataConOrigin :: Text -> Text -> DsM CoreSymbolOrigin
primitiveDataConOrigin moduleName constructorName = do
  PackageId packageName <- gets dsPrimPackageId
  pure (CoreTopLevelOrigin packageName moduleName constructorName)

dataConOriginForType :: TcType -> Text -> DsM CoreSymbolOrigin
dataConOriginForType ty constructorName =
  case ty of
    TcTyCon tyCon _ ->
      pure (CoreTopLevelOrigin (packageIdText (tyConPackageId tyCon)) (tyConModuleName tyCon) constructorName)
    _ -> desugarBug ("data constructor match has no type constructor: " <> show ty)

lookupDataConOrigin :: Text -> DsM CoreSymbolOrigin
...
dsMatches :: TcType -> [Match] -> DsM CoreExpr
dsMatches = dsMatchesWithDicts True

dsMatchesWithDicts :: Bool -> TcType -> [Match] -> DsM CoreExpr
...
dsMatchesWithEnclosingDicts :: [ClassDict] -> TcType -> [Match] -> DsM CoreExpr
...
dsMatchesWithGivenDicts :: [ClassDict] -> TcType -> [Match] -> DsM CoreExpr
dsMatchesWithGivenDicts dicts = dsMatchesWithDictSource [] (Just dicts) False

dsMatchesWithDictSource :: [ClassDict] -> Maybe [ClassDict] -> Bool -> TcType -> [Match] -> DsM CoreExpr
    ...
    let nArgs = length (matchPats m0)
     in if nArgs == 0
          then do
            ...
            let dictLamExpr
                  | abstractDicts = foldr (CoreLam . classDictVar) body dicts
                  | otherwise = body
            pure (foldr CoreTyLam dictLamExpr tyLams)
          else do
            let (tyLams, afterForAlls) = peelForAlls ty
            ...
            let lamExpr = foldr CoreLam body argVars
                dictLamExpr
                  | abstractDicts = foldr (CoreLam . classDictVar) lamExpr dicts
                  | otherwise = lamExpr
            pure (foldr CoreTyLam dictLamExpr tyLams)
  where
...
buildCaseChain :: [Var] -> TcType -> [Match] -> DsM CoreExpr
...
buildCaseChain scrutVars@(scrutVar : restVars) resTy matches
  ...
  | otherwise = do
      ...
      pure (CoreCase (CoreVar scrutVar) caseBinder alts)

dsOrderedMatches :: TcType -> [Var] -> [Match] -> DsM CoreExpr
    ...
    [] -> do
      ...
      pure $
        if isLiftedType resultTy
          then CoreLet (CoreRec [(failureVar, CoreVar failureVar)]) (CoreVar failureVar)
          else CoreVar failureVar
    match : rest -> do
      failure <- dsOrderedMatches resultTy scrutVars rest
      if isLiftedType resultTy
        then do
          failureVar <- freshInternalVar "_next_match" resultTy
          matched <- dsMatchPatterns scrutVars (matchPats match) (dsRhs (matchRhs match)) (CoreVar failureVar)
...
firstPatternKey :: Match -> CoreAltCon
firstPatternKey match =
  case matchPats match of
    pat : _ -> patternKey (PackageId "") pat
    [] -> DefaultAlt

dsMatchPatterns :: [Var] -> [Pattern] -> DsM CoreExpr -> CoreExpr -> DsM CoreExpr
...
dsMatchPattern :: Var -> Pattern -> DsM CoreExpr -> CoreExpr -> DsM CoreExpr
...
dsBoxedCharPatternMatch :: Var -> Char -> DsM CoreExpr -> CoreExpr -> DsM CoreExpr
dsBoxedCharPatternMatch scrutVar char success failure = do
  checkedCharTy <- (`TcTyCon` []) <$> lookupPrimitiveTyCon "GHC.Types" "Char"
  checkedCharHashTy <- (`TcTyCon` []) <$> lookupPrimitiveTyCon "GHC.Prim" "Char#"
  charVar <- freshInternalVar "_char#" checkedCharHashTy
  ...
  let innerCase =
        CoreCase
          (CoreVar charVar)
          innerBinder
          [ CoreAlt (LitAlt (LitChar WordRep char) checkedCharHashTy) [] matched,
            CoreAlt DefaultAlt [] failure
          ]
  pure
    ( CoreCase
        (CoreVar scrutVar)
        outerBinder
        [ CoreAlt (DataAlt (coreConstructorIdFromSymbol charConstructor)) [charVar] innerCase,
          CoreAlt DefaultAlt [] failure
        ]
    )
...
dsOrdinaryPatternMatch :: Var -> Pattern -> DsM CoreExpr -> CoreExpr -> DsM CoreExpr
    ...
    _ -> do
      ...
      pure
        ( CoreCase
            (CoreVar scrutVar)
            caseBinder
            [ CoreAlt con (evidenceBinders <> binders) matched,
              CoreAlt DefaultAlt [] failure
            ]
        )
...
patternKey :: PackageId -> Pattern -> CoreAltCon
    ...
    _ -> do
      ...
      case dsPatternPure packageId pat of
        ...
        (con, binderNames) -> do
          ...
          pure (CoreAlt con (evidenceBinders <> binders) body)
...
dsExpr (EVar name) = do
  ...
  case mLocal of
    Just v -> pure (CoreVar v)
    Nothing -> CoreVar <$> lookupGlobalOccurrence name
dsExpr EInt {} = desugarBug "integer literal does not have a checked type"
dsExpr EChar {} = desugarBug "character literal does not have a checked type"
dsExpr ECharHash {} = desugarBug "primitive character literal does not have a checked type"
dsExpr EString {} = desugarBug "string literal does not have a checked type"
dsExpr EStringHash {} = desugarBug "primitive string literal does not have a checked type"
...
-- occurrence, which 'dsAnnotatedVar' lowers to 'CoreTyApp'.
...
dsAnnotatedVar :: TcAnnotation -> Name -> Expr -> DsM CoreExpr
dsAnnotatedVar tcAnn name _expr = do
  ...
  let occurrenceVar
        ...
        | otherwise = variable
      typedExpr = List.foldl' CoreTyApp (CoreVar occurrenceVar) (tcAnnTypeArgs tcAnn)
  dicts <- mapM dsEvidence (tcAnnEvidenceTerms tcAnn)
  pure (List.foldl' CoreApp typedExpr dicts)
...
dsAnnotatedExpr :: TcAnnotation -> Expr -> DsM CoreExpr
dsAnnotatedExpr tcAnn inner = do
  body <- case inner of
    ...
    EInt value numericType _
      | numericType /= TInteger -> pure (CoreLit (LitInt (numericRuntimeRep numericType) value) (tcAnnType tcAnn))
      | otherwise -> desugarBug "integer literal does not have checked overloaded-literal information"
    EChar char _ -> boxCharLiteralM char
    ECharHash char _ -> pure (CoreLit (LitChar WordRep char) (tcAnnType tcAnn))
    EString text _ -> dsStringLiteral text
    EStringHash text _ -> do
      bytes <- primitiveStringBytes text
      pure (CoreLit (LitAddr bytes) (tcAnnType tcAnn))
  ...
  pure (foldr CoreTyLam body (tcAnnTypeBinders tcAnn))

dsApplication :: Expr -> DsM CoreExpr
dsApplication expression = do
  ...
  case fields of
    ...
    _ -> pure (foldl CoreApp headCore argumentCores)
...
dsStrictConstructorApplication :: CoreExpr -> [Expr] -> [CoreExpr] -> [DataConFieldInfo] -> DsM CoreExpr
dsStrictConstructorApplication headCore argumentExpressions argumentCores fields = do
  ...
  let replacements =
      ...
      application = foldl CoreApp headCore replacements
      strictArguments =
        [ (argumentCore, binder)
  ...
  where
    ...
    replaceStrictArgument maybeBinder argumentCore =
      maybe argumentCore CoreVar maybeBinder

    forceStrictArgument (argumentCore, binder) body =
      CoreCase argumentCore binder [CoreAlt DefaultAlt [] body]

dsDo :: [DoStmt Expr] -> DsM CoreExpr
dsDo stmts =
  case stmts of
...
dsDoBind :: DoStmt Expr -> Expr -> DsM CoreExpr -> DsM CoreExpr
dsDoBind stmt action continuation = do
  ...
  CoreApp (CoreApp bind action') <$> continuation

dsDoPatternContinuation :: Pattern -> [DoStmt Expr] -> DsM CoreExpr
...
dsOverloadedIntegerLiteral :: TcAnnotation -> ResolutionAnnotation -> Integer -> DsM CoreExpr
dsOverloadedIntegerLiteral tcAnn resolution value = do
  ...
  pure (CoreApp fromIntegerExpr integerExpr)

dsIntegerLiteral :: ResolutionAnnotation -> Integer -> DsM CoreExpr
dsIntegerLiteral resolution value = do
  conTy <- lookupType "IS"
  intHashTy <- (`TcTyCon` []) <$> lookupPrimitiveTyCon "GHC.Prim" "Int#"
  ...
  let resolved name var = var {varResolvedName = integerHelperOrigin resolution name}
      small integer = CoreApp (CoreVar con {varResolvedName = Just constructorOrigin}) (CoreLit (LitInt IntRep integer) intHashTy)
      ...
      let buildPositive integer
            ...
            | otherwise =
                let (quotient, remainder) = integer `quotRem` literalBase
                 in CoreApp
                      (CoreApp (CoreVar resolvedAdd) (CoreApp (CoreApp (CoreVar resolvedMultiply) (buildPositive quotient)) (small literalBase)))
                      (small remainder)
          magnitude = buildPositive (abs value)
      pure
        ( if value < 0
            then CoreApp (CoreVar resolvedNegate) magnitude
            else magnitude
        )
  where
...
integerHelperOrigin :: ResolutionAnnotation -> Text -> Maybe CoreSymbolOrigin
integerHelperOrigin resolution symbolName =
  case resolvedNameOrigin (resolutionTarget resolution) of
    Just (CoreTopLevelOrigin packageName _ _) -> Just (CoreTopLevelOrigin packageName "GHC.Internal.Integer" symbolName)
    _ -> Nothing
...
resolvedOccurrenceOrigin :: Name -> Maybe CoreSymbolOrigin
  ...
  case resolvedOccurrenceOrigin name of
    ...
    Just origin -> do
      ...
      case Map.lookup origin globalVariables of
        ...
        Nothing -> desugarBug ("variable use is not in the desugarer environment: " <> T.unpack (coreSymbolOriginText origin))

resolvedNameOrigin :: ResolvedName -> Maybe CoreSymbolOrigin
resolvedNameOrigin resolved =
  case resolved of
    ResolvedTopLevel packageId name ->
      Just
        CoreTopLevelOrigin
          { coreOriginPackage = packageIdText packageId,
            coreOriginModule = fromMaybe "" (nameQualifier name),
            coreOriginName = nameText name
          }
    ResolvedBuiltin name -> Just (CoreBuiltinOrigin name)
...
dsInfix :: Expr -> Name -> Expr -> DsM CoreExpr
dsInfix lhs op rhs = do
  ...
  case fields of
    ...
    _ -> pure (CoreApp (CoreApp operator left) right)

dsInfixOperator :: Name -> DsM CoreExpr
dsInfixOperator op =
  case nameTcAnnotation op of
    Just tcAnn -> dsAnnotatedVar tcAnn op (EVar op)
    Nothing -> dsExpr (EVar op)

dsIf :: Expr -> Expr -> Expr -> DsM CoreExpr
dsIf cond thenE elseE = do
  ...
  pure
    ( CoreCase
        cond'
        binder
        [ CoreAlt (DataAlt (coreConstructorIdFromSymbol trueConstructor)) [] then',
          CoreAlt (DataAlt (coreConstructorIdFromSymbol falseConstructor)) [] else'
        ]
    )

dsCase :: Expr -> [CaseAlt Expr] -> DsM CoreExpr
dsCase scrut alts = do
  scrut' <- dsExpr scrut
  scrutTy <-
    case exprAnnotationType scrut of
      Just ty -> pure ty
      Nothing -> coreExprTypeM scrut'
  case scrut' of
    CoreVar scrutVar -> dsCaseAlternatives scrutVar alts
    _ -> do
...
bindCaseScrutinee :: Var -> CoreExpr -> CoreExpr -> DsM CoreExpr
bindCaseScrutinee scrutVar scrutinee body =
  pure (CoreLet (CoreNonRec scrutVar scrutinee) body)
...
data CaseMatchResult
  = CaseMatchInfallible (DsM CoreExpr)
  | CaseMatchFallible (DsM CoreExpr -> DsM CoreExpr)

dsCaseAlternatives :: Var -> [CaseAlt Expr] -> DsM CoreExpr
...
extractCaseMatchResult :: CaseMatchResult -> DsM CoreExpr -> DsM CoreExpr
...
dsPatternMatchResult :: Var -> Pattern -> DsM CoreExpr -> CaseMatchResult
...
adjustCaseMatchResult :: (CoreExpr -> DsM CoreExpr) -> CaseMatchResult -> CaseMatchResult
...
forceCaseScrutinee :: Var -> CoreExpr -> DsM CoreExpr
forceCaseScrutinee scrutVar body = do
  ...
  pure
    ( CoreCase
        (CoreVar scrutVar)
        caseBinder
        [CoreAlt DefaultAlt [] (substExprVar scrutVar caseBinder body)]
    )

dsOverloadedIntegerPatternMatch :: Var -> Pattern -> DsM CoreExpr -> CoreExpr -> DsM CoreExpr
dsOverloadedIntegerPatternMatch scrutVar pat success failure = do
  test <- dsOverloadedIntegerPatternTest (CoreVar scrutVar) pat
  trueBranch <- success
  testTy <- coreExprTypeM test
  ...
  pure
    ( CoreCase
        ...
        [ CoreAlt (DataAlt (coreConstructorIdFromSymbol trueConstructor)) [] trueBranch,
          CoreAlt (DataAlt (coreConstructorIdFromSymbol falseConstructor)) [] failure
        ]
    )

dsOverloadedIntegerPatternTest :: CoreExpr -> Pattern -> DsM CoreExpr
dsOverloadedIntegerPatternTest scrutValue pat =
  case integerPatternValue pat of
    Just (value, isNegative) -> do
      ...
      let positiveValue = CoreApp fromIntegerExpr integerExpr
      patternValue <-
        if isNegative
          then do
            ...
            pure (CoreApp negateExpr positiveValue)
          else pure positiveValue
      pure (CoreApp (CoreApp eqExpr scrutValue) patternValue)
    Nothing ->
...
dsLetDecls :: [Decl] -> DsM CoreExpr -> DsM CoreExpr
dsLetDecls decls bodyAction = do
  ...
  withLocals localBindings $ do
    ...
    pure $
      if null rhsBindings
        then body
        else CoreLet (CoreRec rhsBindings) body
...
dsLocalGroup :: Var -> LocalDeclGroup -> DsM (Var, CoreExpr)
...
dsStringLiteral :: Text -> DsM CoreExpr
dsStringLiteral text = do
  checkedCharTy <- (`TcTyCon` []) <$> lookupPrimitiveTyCon "GHC.Types" "Char"
  checkedCharHashTy <- (`TcTyCon` []) <$> lookupPrimitiveTyCon "GHC.Prim" "Char#"
  nil <- nilList checkedCharTy
  cons <- consExpr checkedCharTy
  constructorOrigin <- lookupDataConOrigin "C#"
  constructorType <- lookupType "C#"
  pure (T.foldr (applyConsChar constructorOrigin constructorType checkedCharHashTy cons) nil text)
...
dsList :: TcAnnotation -> [Expr] -> DsM CoreExpr
dsList tcAnn elems =
...
dsListComp :: TcAnnotation -> Expr -> [CompStmt] -> DsM CoreExpr
...
dsCompQuals :: TcType -> Expr -> [CompStmt] -> CoreExpr -> DsM CoreExpr
...
dsCompGuard :: TcType -> Expr -> Expr -> [CompStmt] -> CoreExpr -> DsM CoreExpr
dsCompGuard elemTy body guard rest tailExpr = do
  ...
  pure
    ( CoreCase
        ...
        binder
        [ CoreAlt (DataAlt (coreConstructorIdFromSymbol trueConstructor)) [] trueBranch,
          CoreAlt (DataAlt (coreConstructorIdFromSymbol falseConstructor)) [] tailExpr
        ]
    )

dsCompGen :: TcType -> Expr -> Pattern -> Expr -> [CompStmt] -> CoreExpr -> DsM CoreExpr
dsCompGen elemTy body pat src rest tailExpr = do
  src' <- dsExpr src
  srcListTy <- coreExprTypeM src'
  ...
  let recurTail = CoreApp (CoreVar worker) (CoreVar restVar)
  ...
  let workerBody =
        CoreLam listVar $
          CoreCase
            (CoreVar listVar)
            caseBinder
            [ CoreAlt (DataAlt (coreConstructorIdFromSymbol nilConstructor)) [] tailExpr,
              CoreAlt (DataAlt (coreConstructorIdFromSymbol consConstructor)) [headVar, restVar] consRhs
            ]
  pure (CoreLet (CoreRec [(worker, workerBody)]) (CoreApp (CoreVar worker) src'))

dsCompGenMatch :: TcType -> Expr -> Pattern -> [CompStmt] -> Var -> CoreExpr -> DsM CoreExpr
...
dsLambda :: [Pattern] -> Expr -> DsM CoreExpr
dsLambda pats body = do
  ...
  pure (foldr CoreLam body' vars)

dsLambdaCase :: [CaseAlt Expr] -> DsM CoreExpr
dsLambdaCase alts =
  case alts of
    firstAlt : _ -> do
      ...
      pure (CoreLam argVar body)
    [] -> desugarBug "cannot desugar an empty lambda-case"

dsLambdaCases :: [LambdaCaseAlt] -> DsM CoreExpr
dsLambdaCases alts =
  case alts of
    firstAlt : _ -> do
      ...
      pure (foldr CoreLam body argVars)
    [] -> desugarBug "cannot desugar an empty multi-argument lambda-case"

dsLambdaCaseAlternatives :: [Var] -> [LambdaCaseAlt] -> DsM CoreExpr
...
dsTuple :: TupleFlavor -> TcAnnotation -> [Maybe Expr] -> DsM CoreExpr
...
tupleConExpr flavor elemTys = do
  ...
  (constructorTy, resolvedOrigin) <-
    case (flavor, constructorOrigin) of
      (_, Just origin@CoreTopLevelOrigin {}) -> (,Just origin) <$> lookupType name
      (Unboxed, _) -> do
  ...
  constructor <-
    case resolvedOrigin of
      Just CoreBuiltinOrigin {} -> pure (builtinVar name (Unique (-20 - arity)) constructorTy)
  ...
  pure (List.foldl' CoreTyApp (CoreVar constructor {varResolvedName = resolvedOrigin}) typeArguments)
...
withTupleConstructorOrigin :: Maybe CoreSymbolOrigin -> DsM a -> DsM a
...
applyConsChar :: CoreSymbolOrigin -> TcType -> TcType -> CoreExpr -> Char -> CoreExpr -> CoreExpr
applyConsChar constructorOrigin constructorType checkedCharHashTy cons char =
  applyCons cons (boxCharLiteral constructorOrigin constructorType checkedCharHashTy char)

boxCharLiteral :: CoreSymbolOrigin -> TcType -> TcType -> Char -> CoreExpr
boxCharLiteral constructorOrigin constructorType checkedCharHashTy char =
  CoreApp
    ( CoreVar
        ( (Var "C#" (Unique (-12)) constructorType)
            { varResolvedName = Just constructorOrigin
            }
        )
    )
    (CoreLit (LitChar WordRep char) checkedCharHashTy)

boxCharLiteralM :: Char -> DsM CoreExpr
boxCharLiteralM char = do
  constructorOrigin <- lookupDataConOrigin "C#"
  constructorType <- lookupType "C#"
  checkedCharHashTy <- (`TcTyCon` []) <$> lookupPrimitiveTyCon "GHC.Prim" "Char#"
  pure (boxCharLiteral constructorOrigin constructorType checkedCharHashTy char)

applyCons :: CoreExpr -> CoreExpr -> CoreExpr -> CoreExpr
applyCons cons headExpr =
  CoreApp (CoreApp cons headExpr)

nilList :: TcType -> DsM CoreExpr
nilList elemTy = do
  constructor <- listConstructorVar "[]" (Unique (-10))
  pure (CoreTyApp (CoreVar constructor) elemTy)

consExpr :: TcType -> DsM CoreExpr
consExpr elemTy = do
...
listConstructorVar name unique = do
  ...
  pure
    (Var name unique constructorType)
      { varResolvedName = Just (CoreTopLevelOrigin (packageIdText packageId) "GHC.Types" name)
      }

builtinVar :: Text -> Unique -> TcType -> Var
builtinVar name unique ty =
  (Var name unique ty) {varResolvedName = Just (CoreBuiltinOrigin name)}

listType :: TcType -> TcType
listType ty =
  TcTyCon (TyCon "[]" 1) [ty]

dsEvidence :: EvTerm -> DsM CoreExpr
dsEvidence evidence =
  case evidence of
    EvGiven (ClassPred classTyCon args) -> do
      st <- get
      case Map.lookup (exactDictKey classTyCon args) (dsLocalDicts st) of
        Just var -> pure (CoreVar var)
        Nothing ->
    ...
    EvDict dictOrigin dictName typeArgs contextEvidence -> do
      ...
      let origin = fmap (\(packageName, moduleName) -> CoreTopLevelOrigin packageName moduleName dictName) dictOrigin
          dictExpr = List.foldl' CoreTyApp (CoreVar (Var dictName (Unique (-199)) dictTy) {varResolvedName = origin}) typeArgs
      pure (List.foldl' CoreApp dictExpr contextDicts)
    EvCoercion {} ->
      unitConstructor
    EvSuperClass source sourceOrigin sourcePredicate fieldTypes fieldIndex -> do
      ...
      constructor <-
        case sourcePredicate of
          ClassPred classTyCon _ -> pure (coreDictionaryConstructorName (tyConName classTyCon))
          EqPred {} -> desugarBug "cannot select a superclass from equality evidence"
      let constructorOrigin =
            case sourceOrigin of
              Just (packageName, moduleName) -> CoreTopLevelOrigin packageName moduleName constructor
              Nothing -> CoreBuiltinOrigin constructor
      pure (CoreCase sourceExpression sourceBinder [CoreAlt (DataAlt (coreConstructorIdFromSymbol constructorOrigin)) fieldBinders (CoreVar selected)])
    EvCast dict _co ->
      dsEvidence dict
...
dsTypeableEvidence :: Maybe (Text, Text) -> TcType -> [EvTerm] -> DsM CoreExpr
dsTypeableEvidence typeableOrigin ty argumentEvidence = do
  ...
  if length argumentTypes /= length argumentEvidence
    then desugarBug "Typeable evidence argument arity mismatch"
      ...
      dictionaryOrigin <- typeableConstructorOrigin typeableOrigin (coreDictionaryConstructorName "Typeable")
      let proxyMethod = CoreLam proxy representation
          valueMethod = CoreLam value representation
          dictionaryConstructor =
            ( Var
                (coreDictionaryConstructorName "Typeable")
                (Unique (-2100))
                ( TcForAllTy
                    typeableTyVar
                    ( TcFunTy
      ...
      pure (CoreApp (CoreApp (CoreTyApp (CoreVar dictionaryConstructor) ty) proxyMethod) valueMethod)

dsTypeableArgument :: Maybe (Text, Text) -> TcType -> EvTerm -> DsM CoreExpr
dsTypeableArgument typeableOrigin ty evidence = do
  ...
  let proxyConstructor =
      ...
      proxy = CoreTyApp (CoreVar proxyConstructor) ty
  pure
    ( CoreCase
        dictionary
        dictionaryBinder
        [ CoreAlt
            (DataAlt (coreConstructorIdFromSymbol dictionaryOrigin))
            [proxyMethod, valueMethod]
            (CoreApp (CoreVar proxyMethod) proxy)
        ]
    )

dsTypeRepresentation :: Maybe (Text, Text) -> TcType -> [CoreExpr] -> DsM CoreExpr
dsTypeRepresentation typeableOrigin ty arguments =
  case typeableTypeView ty of
    ...
    Just (name, _) -> do
      checkedCharTy <- (`TcTyCon` []) <$> lookupPrimitiveTyCon "GHC.Types" "Char"
      checkedCharHashTy <- (`TcTyCon` []) <$> lookupPrimitiveTyCon "GHC.Prim" "Char#"
      charNil <- nilList checkedCharTy
      charCons <- consExpr checkedCharTy
      charConstructorOrigin <- lookupDataConOrigin "C#"
      charConstructorType <- lookupType "C#"
      ...
      let tyConConstructor =
          ...
          tyCon = CoreApp (CoreVar tyConConstructor) (T.foldr (applyConsChar charConstructorOrigin charConstructorType checkedCharHashTy charCons) charNil name)
          argumentList = foldr (applyCons argumentCons) argumentNil arguments
      pure (CoreApp (CoreApp (CoreVar typeRepConstructor) tyCon) argumentList)

typeableConstructorOrigin :: Maybe (Text, Text) -> Text -> DsM CoreSymbolOrigin
typeableConstructorOrigin origin constructorName =
  case origin of
    Just (packageName, _) | constructorName == "Proxy" -> pure (CoreTopLevelOrigin packageName "Data.Proxy" constructorName)
    Just (packageName, moduleName) -> pure (CoreTopLevelOrigin packageName moduleName constructorName)
...
unitConstructor :: DsM CoreExpr
unitConstructor = do
  ty <- lookupType "()"
  pure (CoreVar (Var "()" (Unique (-13)) ty))
...
coreExprTypeM :: CoreExpr -> DsM TcType
coreExprTypeM expr =
  case expr of
    CoreVar var -> pure (varType var)
    CoreLit _ ty -> pure ty
    CoreApp fun _arg -> do
      funTy <- coreExprTypeM fun
      case funTy of
    ...
    CoreLam var body -> TcFunTy (varType var) <$> coreExprTypeM body
    CoreTyLam tv body -> TcForAllTy tv <$> coreExprTypeM body
    CoreLet _bind body -> coreExprTypeM body
    CoreCase _scrut _binder alts ->
      case alts of
        [] -> desugarBug "case expression has no alternatives while desugaring"
        CoreAlt _ _ body : _ -> coreExprTypeM body
    CoreCast inner _co -> coreExprTypeM inner
    CoreCallForeign foreignCall _arguments ->
      pure (coreForeignCallResultType (coreForeignCallSignature foreignCall))
