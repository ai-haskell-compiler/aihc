...
-- | Convert the type-checked surface AST to private FC2 desugarer terms.
module Aihc.Fc2.Desugar.Core
  ( -- * Entry point
...
import Aihc.Fc2.Desugar.Core.Deriving (dsDerivingPlans, moduleDerivingPlans)
import Aihc.Fc2.Desugar.Core.Dictionary (checkedConstraintType, classMethodFieldType, defaultMethodName, peelForAlls, peelQuals, predType)
import Aihc.Fc2.Desugar.Core.Expr (ClassDict (..), DsM, DsState (..), desugarBug, dsEvidence, dsMatches, dsMatchesWithEnclosingDicts, freshUnique, freshVar, lookupType, withDicts)
import Aihc.Fc2.Desugar.Core.External (declareExternalSymbols)
import Aihc.Fc2.Desugar.Core.Lower (lowerPseudoOps)
import Aihc.Fc2.Desugar.Core.Match (dsDataConPure)
import Aihc.Fc2.Desugar.Core.Newtype (lowerNewtypes)
import Aihc.Fc2.Desugar.Core.Subst (freeRigidTyVars, substType)
...
desugarModule :: DesugarConfig -> [TcBindingResult] -> [DataTypeInfo] -> Map.Map (PackageId, Text, Text) TyCon -> Map.Map CoreSymbolOrigin Var -> Module -> DesugarResult
desugarModule config bindings dataTypes globalTyConEnv globalVars tcResult =
  if not (tcModuleSuccess tcResult)
    then
      DesugarResult
        { dsProgram = CoreProgram (sourceModuleId tcResult) [],
          dsSuccess = False,
      ...
      let typeEnv = Map.fromList (concatMap bindingTypeEntries bindings)
       ...
       in case runStateT (dsModule tcResult) initialState of
            ...
            Right (binds, _) ->
              let program = CoreProgram (CoreModuleId packageId currentModuleName) binds
               in case lowerConstraintProgram (lowerNewtypes program) of
                    Left err ->
                      DesugarResult
                        { dsProgram = CoreProgram (CoreModuleId packageId currentModuleName) [],
                          dsSuccess = False,
                          dsErrors = [err]
                        }
...
interfaceGlobalVars :: TcInterface -> Map.Map CoreSymbolOrigin Var
interfaceGlobalVars interface =
  Map.fromList
    [ (origin, coreExternalVar origin (typeSchemeType scheme))
    ...
    termOrigin key =
      case key of
        ...
        TcTermGlobal packageId moduleName' identifier
          | packageId == PackageId "" && moduleName' == "" -> Just (CoreBuiltinOrigin identifier)
          | otherwise -> Just (CoreTopLevelOrigin (packageIdText packageId) moduleName' identifier)
...
sourceModuleId :: Module -> CoreModuleId
sourceModuleId modu = CoreModuleId "" (fromMaybe "Main" (moduleName modu))
...
lowerConstraintProgram :: CoreProgram -> Either String CoreProgram
lowerConstraintProgram (CoreProgram moduleId topBinds) =
  CoreProgram moduleId <$> mapM lowerTopBind topBinds
  where
    lowerTopBind topBind =
      case topBind of
        CoreExternal origin ty -> CoreExternal origin <$> lowerConstraintType ty
        CoreData declaration ->
          CoreData . (\constructors -> declaration {coreDataConstructors = constructors})
            <$> mapM lowerDataConstructor (coreDataConstructors declaration)
        CoreAxiom declaration ->
          do
            left <- lowerConstraintType (coreAxiomLeft declaration)
            right <- lowerConstraintType (coreAxiomRight declaration)
            pure (CoreAxiom declaration {coreAxiomLeft = left, coreAxiomRight = right})
        CoreNewtype declaration ->
          do
            representation <- lowerConstraintType (coreNewtypeRepresentation declaration)
            result <- lowerConstraintType (coreNewtypeResult declaration)
            pure (CoreNewtype declaration {coreNewtypeRepresentation = representation, coreNewtypeResult = result})
        CorePrimitive var arity -> (`CorePrimitive` arity) <$> lowerVar var
        CoreForeignImport foreignCall -> pure (CoreForeignImport foreignCall)
        CoreTopBind bind -> CoreTopBind <$> lowerBind bind

    lowerDataConstructor constructor = do
      fields <- mapM lowerConstraintType (coreDataConFields constructor)
      pure constructor {coreDataConFields = fields}

    lowerBind bind =
      case bind of
        CoreNonRec var expression -> CoreNonRec <$> lowerVar var <*> lowerExpr expression
        CoreRec bindings -> CoreRec <$> mapM (\(var, expression) -> (,) <$> lowerVar var <*> lowerExpr expression) bindings

    lowerExpr expression =
      case expression of
        CoreVar var -> CoreVar <$> lowerVar var
        CoreLit {} -> pure expression
        CoreApp function argument -> CoreApp <$> lowerExpr function <*> lowerExpr argument
        CoreTyApp function ty -> CoreTyApp <$> lowerExpr function <*> lowerConstraintType ty
        CoreLam var body -> CoreLam <$> lowerVar var <*> lowerExpr body
        CoreTyLam tyVar body -> CoreTyLam tyVar <$> lowerExpr body
        CoreLet bind body -> CoreLet <$> lowerBind bind <*> lowerExpr body
        CoreCase scrutinee binder alternatives ->
          CoreCase <$> lowerExpr scrutinee <*> lowerVar binder <*> mapM lowerAlt alternatives
        CoreCast inner coercion -> CoreCast <$> lowerExpr inner <*> lowerCoercion coercion
        CoreCallForeign foreignCall arguments -> CoreCallForeign foreignCall <$> mapM lowerExpr arguments
...
dsModule :: Module -> DsM [CoreTopBind]
dsModule m = do
  ...
  let valueTops = [CoreTopBind (CoreRec valueBindings) | not (null valueBindings)]
...
dsDataDeclM :: DataDecl -> DsM [CoreTopBind]
dsDataDeclM dd = do
  ...
  pure (CoreData (CoreDataDecl tyOrigin tyName typeVariables resultKind constructors) : selectors)
...
checkedDataTypeOrigin :: DataTypeInfo -> CoreSymbolOrigin
checkedDataTypeOrigin info =
  let tyCon = dtiTyCon info
   in CoreTopLevelOrigin (packageIdText (tyConPackageId tyCon)) (tyConModuleName tyCon) (dtiName info)
...
checkedFcDataCon :: DataConInfo -> CoreDataConDecl
checkedFcDataCon info =
  let (packageId, moduleName') = dciOrigin info
      origin = CoreTopLevelOrigin (packageIdText packageId) moduleName' (dciName info)
   in CoreDataConDecl (coreConstructorIdFromSymbol origin) (dciName info) (map predType (dciTheta info) <> map dcfiType (dciFields info))
...
dsDataFamilyInstM :: DataFamilyInstanceInfo -> DataFamilyInst -> DsM [CoreTopBind]
    ...
    _ -> do
      let representationTyCon = dfiiRepresentationTyCon familyInfo
          ...
          axiom =
            CoreAxiom
              CoreAxiomDecl
                { coreAxiomName = dfiiAxiomName familyInfo,
                  coreAxiomTyVars = representationTyVars,
                  coreAxiomRole = CoreNominal,
                  coreAxiomLeft = dfiiFamilyType familyInfo,
                  coreAxiomRight = representationType
                }
...
dataFamilyRepresentation :: DataFamilyInst -> Text -> [TyVarId] -> TcType -> [(Text, [TyVarId], [TcType], TcType)] -> DsM CoreTopBind
dataFamilyRepresentation familyInst representationName representationTyVars representationType constructorInfos
      ...
      case constructorInfos of
        [(constructorName, _, [fieldType], _)] -> do
          ...
          pure
            ( CoreNewtype
                CoreNewtypeDecl
                  { coreNewtypeOrigin = newtypeOrigin,
                    coreNewtypeName = representationName,
                    coreNewtypeTyVars = representationTyVars,
                    coreNewtypeConstructorOrigin = coreConstructorIdFromSymbol constructorOrigin,
                    coreNewtypeConstructor = constructorName,
                    coreNewtypeRepresentation = fieldType,
                    coreNewtypeResult = representationType
                  }
            )
  ...
  | otherwise =
      do
        ...
        constructors <- mapM (\(name, _, fields, _) -> (CoreDataConDecl . coreConstructorIdFromSymbol <$> localDeclarationOrigin name) <*> pure name <*> pure fields) constructorInfos
        pure
          ( CoreData
              (CoreDataDecl dataOrigin representationName representationTyVars (typeKind representationType) constructors)
          )
...
dsNewtypeDeclM :: NewtypeDecl -> DsM [CoreTopBind]
      ...
      case (dciTheta constructorInfo, dciFields constructorInfo) of
        ([], [field]) -> do
          let conName = dciName constructorInfo
              ...
              constructorOrigin = coreDataConOrigin (checkedFcDataCon constructorInfo)
          ...
          pure
            ( CoreNewtype
                CoreNewtypeDecl
                  { coreNewtypeOrigin = checkedDataTypeOrigin dataType,
                    coreNewtypeName = tyName,
                    coreNewtypeTyVars = tyVars,
                    coreNewtypeConstructorOrigin = constructorOrigin,
                    coreNewtypeConstructor = conName,
                    coreNewtypeRepresentation = fieldTy,
                    coreNewtypeResult = resultTy
                  }
                : selectors
            )
...
dsRecordSelectors :: [(Text, [TyVarId], [TcType])] -> [DataConDecl] -> DsM [CoreTopBind]
  ...
  where
    ...
    dsSelector (selectorName, layouts) = do
      ...
      let needsDefault = length matchingAlternatives < length constructorInfos
          failure = CoreCase (CoreVar recordVar) failureBinder []
          completeAlternatives =
            matchingAlternatives
              <> [CoreAlt DefaultAlt [] failure | needsDefault]
          selection = CoreCase (CoreVar recordVar) caseBinder completeAlternatives
          body = foldr CoreTyLam (foldr CoreLam (CoreLam recordVar selection) dictionaryVars) typeVariables
      pure (CoreTopBind (CoreNonRec selectorVar body))
      ...
      case lookup constructorName layouts of
        ...
        Just fieldIndex -> do
          ...
          case drop selectedIndex fieldBinders of
            selected : _ -> pure (Just (CoreAlt (DataAlt (coreConstructorIdFromSymbol constructorOrigin)) fieldBinders (CoreVar selected)))
...
dsForeignImport :: TcAnnotation -> Maybe TcForeignImportAnnotation -> ForeignDecl -> DsM [CoreTopBind]
...
dsForeignPrim :: TcAnnotation -> ForeignDecl -> DsM CoreTopBind
dsForeignPrim tcAnn foreignDecl = do
  ...
  unique <- freshUnique
  pure (CorePrimitive (Var name unique ty) arity)

dsForeignCcall :: TcAnnotation -> TcForeignImportAnnotation -> ForeignDecl -> DsM [CoreTopBind]
  ...
  let name = unqualifiedNameText (foreignName foreignDecl)
      ...
      signature =
        CoreForeignSignature
          { coreForeignArgumentTypes = map (lowerForeignAbiType . tcForeignAbiType) (tcForeignArguments foreignPlan),
            coreForeignResultType = lowerForeignAbiType (tcForeignAbiType (tcForeignResult foreignPlan)),
            coreForeignEffect = lowerForeignEffect (tcForeignEffect foreignPlan)
          }
      foreignCall =
        CoreForeignCall
          { coreForeignCallName = "$ffi$" <> name,
            coreForeignCallSymbol = symbol,
            coreForeignCallSignature = signature
          }
  ...
  wrapperBody <-
    unboxForeignArguments (zip argumentVars (tcForeignArguments foreignPlan)) $ \arguments ->
      case tcForeignEffect foreignPlan of
        TcForeignPure ->
          boxForeignValue (tcForeignResult foreignPlan) (CoreCallForeign foreignCall arguments)
        TcForeignRealWorld -> makeForeignIoWrapper foreignCall (tcForeignResult foreignPlan) arguments
  pure
    [ CoreForeignImport foreignCall,
      CoreTopBind (CoreNonRec wrapperVar (foldr CoreLam wrapperBody argumentVars))
    ]

lowerForeignAbiType :: TcForeignAbiType -> CoreForeignType
lowerForeignAbiType foreignType =
  case foreignType of
    TcForeignInt -> CoreForeignInt
    TcForeignInt32 -> CoreForeignInt32
    TcForeignWord64 -> CoreForeignWord64
    TcForeignAddr -> CoreForeignAddr

lowerForeignEffect :: TcForeignEffect -> CoreForeignEffect
lowerForeignEffect effect =
  case effect of
    TcForeignPure -> CoreForeignPure
    TcForeignRealWorld -> CoreForeignRealWorld

unboxForeignArguments :: [(Var, TcForeignMarshal)] -> ([CoreExpr] -> DsM CoreExpr) -> DsM CoreExpr
unboxForeignArguments arguments continuation = go arguments []
  where
    go [] values = continuation (reverse values)
    go ((var, marshal) : rest) values =
      unboxForeignValue (varName var) marshal (CoreVar var) $ \value -> go rest (value : values)

unboxForeignValue :: Text -> TcForeignMarshal -> CoreExpr -> (CoreExpr -> DsM CoreExpr) -> DsM CoreExpr
unboxForeignValue binderName marshal expression continuation =
  ...
  where
    ...
    go valueType (constructor : constructors) value = do
      ...
      rhs <- go fieldType constructors (CoreVar fieldBinder)
      constructorOrigin <- typeDataConOrigin valueType constructor
      pure
        ( CoreCase
            value
            caseBinder
            [CoreAlt (DataAlt (coreConstructorIdFromSymbol constructorOrigin)) [fieldBinder] rhs]
        )

boxForeignValue :: TcForeignMarshal -> CoreExpr -> DsM CoreExpr
boxForeignValue marshal =
  ...
  where
    ...
    applyConstructors resultType (constructor : constructors) value = do
      ...
      pure (CoreApp (foldl CoreTyApp (CoreVar constructorVar {varResolvedName = Just constructorOrigin}) typeArguments) fieldValue)
...
makeForeignIoWrapper :: CoreForeignCall -> TcForeignMarshal -> [CoreExpr] -> DsM CoreExpr
makeForeignIoWrapper foreignCall resultMarshal arguments = do
  stateVar <- freshVar "$ffi_state" statePrimRealWorldTy
  tupleBinder <- freshVar "$ffi_result" (coreForeignCallResultType (coreForeignCallSignature foreignCall))
  ...
  boxedResult <- boxForeignValue resultMarshal (CoreVar rawResultVar)
  ...
  let resultTuple = CoreApp (CoreApp (CoreVar tupleConstructor) (CoreVar nextStateVar)) boxedResult
      call = CoreCallForeign foreignCall (arguments <> [CoreVar stateVar])
      stateAction =
        CoreLam
          stateVar
          ( CoreCase
              call
              tupleBinder
              [CoreAlt (DataAlt (coreConstructorIdFromSymbol tupleOrigin)) [nextStateVar, rawResultVar] resultTuple]
          )
  pure (CoreApp (CoreTyApp (CoreVar ioConstructor) (tcForeignSourceType resultMarshal)) stateAction)
...
dsClassDeclM :: ClassDecl -> TcClassAnnotation -> DsM [CoreTopBind]
dsClassDeclM classDecl classAnn = do
  ...
  let dictionaryDeclaration = CoreData (CoreDataDecl classOrigin className classTyVars KType [CoreDataConDecl (coreConstructorIdFromSymbol constructorOrigin) dictionaryConstructor fieldTypes])
  ...
  where
    ...
    dictionaryConstructor = coreDictionaryConstructorName className

localDeclarationOrigin :: Text -> DsM CoreSymbolOrigin
localDeclarationOrigin declarationName = do
  packageId <- gets dsModulePackage
  moduleName' <- gets dsModuleName
  pure (CoreTopLevelOrigin (packageIdText packageId) moduleName' declarationName)

primitiveConstructorOrigin :: Text -> DsM CoreSymbolOrigin
primitiveConstructorOrigin declarationName = do
  packageId <- gets dsPrimPackageId
  pure (CoreTopLevelOrigin (packageIdText packageId) "GHC.Types" declarationName)

typeDataConOrigin :: TcType -> Text -> DsM CoreSymbolOrigin
...
dsClassSelector :: Text -> Int -> [TyVarId] -> [TcType] -> TcClassMethodAnnotation -> DsM CoreTopBind
dsClassSelector dictionaryConstructor superClassCount classTyVars fieldTypes methodAnn = do
  ...
  let extraTyVars = filter (`notElem` classTyVars) (tcClassMethodTyVars methodAnn)
      ...
      selected =
        foldl
          CoreApp
          (foldl CoreTyApp (CoreVar selectedField) (map TcTyVar extraTyVars))
          (map CoreVar extraDictVars)
      selection = CoreCase (CoreVar classDictionaryVar) caseBinder [CoreAlt (DataAlt (coreConstructorIdFromSymbol constructorOrigin)) fieldBinders selected]
      methodVar = Var (tcClassMethodName methodAnn) methodUnique (tcClassMethodType methodAnn)
      body = foldr CoreTyLam (foldr CoreLam selection dictVars) (tcClassMethodTyVars methodAnn)
  pure (CoreTopBind (CoreNonRec methodVar body))
  where
...
dsClassDefault :: (TcInstanceMethodAnnotation, [Match]) -> DsM CoreTopBind
dsClassDefault (methodAnn, matches) = do
  ...
  pure (CoreTopBind (CoreNonRec worker body))
...
dsInstanceDecl :: Decl -> DsM [CoreTopBind]
dsInstanceDecl decl =
  case decl of
...
dsInstanceDict :: TcInstanceAnnotation -> InstanceDecl -> DsM CoreTopBind
dsInstanceDict instAnn instanceDecl = do
  ...
  let methodOrder = tcInstanceMethodOrder instAnn
      ...
      selfDictionary dictVar =
        foldl
          CoreApp
          (foldl CoreTyApp (CoreVar dictVar) (map TcTyVar (tcInstanceTyVars instAnn)))
      ...
      buildDictionary recursive dictVar fields = do
        ...
        let dictionaryConstructor = coreDictionaryConstructorName className
            ...
            constructor = foldl CoreTyApp (CoreVar constructorVar) (tcInstanceHeadTypes instAnn)
            dictionary = foldl CoreApp constructor fields
            dictBody = foldr CoreTyLam (foldr (CoreLam . classDictVar) dictionary contextDicts) (tcInstanceTyVars instAnn)
            bindingGroup
              | recursive = CoreRec [(dictVar, dictBody)]
              | otherwise = CoreNonRec dictVar dictBody
        pure (CoreTopBind bindingGroup)
...
dsInstanceMethod :: [ClassDict] -> Map.Map Text (TcType, [Match]) -> Maybe CoreExpr -> [TcType] -> Maybe (Text, Text) -> [Text] -> Text -> DsM CoreExpr
  ...
  case Map.lookup methodName methods of
    ...
    Nothing
      | methodName `elem` defaults -> do
          ...
          let origin = fmap (\(packageName, originModule) -> CoreTopLevelOrigin packageName originModule workerName) classOrigin
          pure (CoreApp (foldl CoreTyApp (CoreVar worker {varResolvedName = origin}) headTypes) selfDictionary)
      | otherwise ->
...
dsGroup :: (DeclGroup, Var) -> DsM (Var, CoreExpr)
dsGroup (grp, var) = do
  body <-
    case grp of
