...
prop_modulePrettyRoundTrip modu =
  let source = renderStrict (layoutPretty defaultLayoutOptions (pretty modu))
   in if isGhcUnstableCompTransformSource source
        then property True
        else
          let (errs, reparsed) = parseModule moduleConfig source
  ...
  let source = renderStrict (layoutPretty defaultLayoutOptions (pretty modu))
   in if isGhcUnstableCompTransformSource source
        then property True
          ...
          let mbErr = validateParser "<quickcheck>" GHC2024Edition (map EnableExtension requiredExtensions) source
           in counterexample ("Original source:\n" <> T.unpack source) $
                case mbErr of
...
isGhcUnstableCompTransformSource :: T.Text -> Bool
isGhcUnstableCompTransformSource source =
  T.isInfixOf "then (- #" source
    || T.isInfixOf "then group by (- #" source
