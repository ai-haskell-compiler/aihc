...
typeConfig =
  defaultConfig
    { parserExtensions = [UnboxedTuples, UnboxedSums, TemplateHaskell, ImplicitParams]
    }
...
normalizeType ty =
  case ty of
    ...
    TParen _ (TConstraintWildcard _) -> TWildcard span0
    -- TParen around TTuple/TUnboxedSum/TList is also stripped (recursively)
    ...
    -- TParen around TSplice - strip and normalize expression
    TParen _ (TSplice _ body) -> TSplice span0 (normalizeExpr body)
