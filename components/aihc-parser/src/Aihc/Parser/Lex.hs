...
closeBeforeToken st tok =
  ...
  where
    closeBeforeThenElse =
      let col = tokenStartCol tok
       in case lexTokenKind tok of
            TkKeywordThen ->
              closeImplicitLayouts (lexTokenSpan tok) (\indent _ -> col < indent)
            TkKeywordElse ->
              closeImplicitLayouts (lexTokenSpan tok) $
                \indent kind ->
                  col < indent
                    || case kind of
                      LayoutAfterThen ifDepth -> col == indent && ifDepth == layoutIfDepth st
                      _ -> False
            _ -> noLayoutClosures
  ...
  case lexTokenKind tok of
    TkKeywordDo
      | layoutPrevTokenKind st == Just TkKeywordThen ->
          st {layoutPendingLayout = Just (LayoutAfterThen (layoutIfDepth st))}
      | layoutPrevTokenKind st == Just TkKeywordElse ->
          st {layoutPendingLayout = Just LayoutAfterElse}
