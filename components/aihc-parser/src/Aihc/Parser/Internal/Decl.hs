...
exportNameParser mWarning = do
  ...
  pure $ \span' ->
    case members of
      ...
      Nothing
        | namespace == Just ExportNamespaceType || isTypeName name ->
            ExportAbs span' mWarning namespace name
        | otherwise ->
            ExportVar span' mWarning namespace name

exportMembersParser :: TokParser (Maybe [ExportMember])
exportMembersParser =
  parens $
...
importItemParser = withSpan $ do
  ...
  pure $ \span' ->
    case members of
      ...
      Nothing
        | effectiveNamespace == Just ExportNamespaceType || isTypeName itemName -> ImportItemAbs span' effectiveNamespace itemName
...
exportImportNamespaceParser :: TokParser ExportNamespace
exportImportNamespaceParser =
  (expectedTok TkKeywordType >> pure ExportNamespaceType)
    <|> (expectedTok TkKeywordData >> pure ExportNamespaceData)
    <|> patternNamespaceParser
  where
    patternNamespaceParser = do
      patSynEnabled <- isExtensionEnabled PatternSynonyms
      if patSynEnabled
        then expectedTok TkVarPattern >> pure ExportNamespacePattern
        else MP.empty

bundledNamespaceParser :: TokParser ExportMemberNamespace
bundledNamespaceParser =
  expectedTok TkKeywordData >> pure ExportMemberNamespaceData
