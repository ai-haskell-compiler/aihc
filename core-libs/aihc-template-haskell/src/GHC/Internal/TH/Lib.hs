...
infixE (Just x) s (Just y) = do
  a <- x
  s' <- s
  b <- y
  pure (InfixE (Just a) s' (Just b))
infixE Nothing s (Just y) = do
  s' <- s
  b <- y
  pure (InfixE Nothing s' (Just b))
infixE (Just x) s Nothing = do
  a <- x
  s' <- s
