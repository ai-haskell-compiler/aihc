...
newNameIO s = do
  n <- atomicModifyIORef' counter (\x -> (x + 1, x))
  pure (mkNameU s n)

badIO :: String -> IO a
badIO op = do
  qReport True ("Can't do `" ++ op ++ "' in the IO monad")
...
isInstance nm tys = do
  decs <- reifyInstances nm tys
...
data NameSpace
  ...
  | -- | Data constructors
    DataName
  | -- | Type constructors and classes; Haskell has them
...
mkNameG_fld ::
  ...
  -- | module
  String ->
  -- | parent (first constructor of parent type)
  String ->
  -- | field name
  String ->
  Name
...
data Lit
  ...
  | -- | @"string"@
    StringL String
  | -- | @123@. Used for overloaded and non-overloaded
  ...
  | -- | @"string"#@. A primitive C-style string, type 'Addr#'
    StringPrimL [Word8]
...
data Exp
  ...
  | -- | @{ case e of m1; m2 }@
    CaseE Exp [Match]
  | -- | @{ do { p <- e1; e2 }  }@ or a qualified do if
    -- the module name is present
    DoE (Maybe ModName) [Stmt]
  | -- | @{ mdo { x <- e1 y; y <- e2 x; } }@ or a qualified
...
data NamespaceSpecifier
  = -- | Name may be everything; If there are two
    --   names in different namespaces, then consider both
    NoNamespaceSpecifier
  | -- | Name should be a type-level entity, such as a
...
data Overlap
  ...
  | -- | Both 'Overlapping' and 'Overlappable'
    Overlaps
  | -- | Both 'Overlapping' and 'Overlappable', and
...
data DocLoc
  ...
  | -- | At a declaration, not necessarily top level.
    DeclDoc Name
  | -- | At a specific argument of a function, indexed by its
    -- position.
    ArgDoc Name Int
