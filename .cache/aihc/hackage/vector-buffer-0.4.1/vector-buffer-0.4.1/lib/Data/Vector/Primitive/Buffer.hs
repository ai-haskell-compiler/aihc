...
pushNextElement :: M.Prim a => Buffer a -> a -> IO ()
...
-- | convert to a vector
toVector :: M.Prim a => Buffer a -> IO (V.Vector a)
toVector (B o n v) = do
   w <- M.new n
   i <- readIORef o
   let n' = n-i
...
mapBufferM :: (M.Prim a, M.Prim b) => (a -> IO b) -> Buffer a -> IO (V.Vector b)
...
-- | monadic map over a buffer
mapBufferM_ :: M.Prim a => (a -> IO b) -> Buffer a -> IO ()
