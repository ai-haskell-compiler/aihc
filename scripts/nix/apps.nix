{
  ...
  evalExtensionProgressExe = pkgs.lib.getExe' hsPkgs.aihc-testing "eval-extension-progress";
...
in {
  ...
  eval-extension-progress = mkAppWithInputs "eval-extension-progress" [pkgs.bash] ''
    set -euo pipefail
    ${repoRootGuard}
    exec ${evalExtensionProgressExe} "$@"
  '';
